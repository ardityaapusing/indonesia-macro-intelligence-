...
server <- function(input, output, session) {
  ...
  output$prov_bar <- renderPlotly({
    var_sel <- input$prov_var
    df <- provincial_gdp %>% arrange(desc(!!sym(var_sel))) %>% head(15)
    val <- df[[var_sel]]
    lbl <- switch(var_sel,"pdrb"="Triliun Rp","growth"="% Growth","gini"="Gini")
    p <- plot_ly(df %>% arrange(!!sym(var_sel)),
      x=val, y=~reorder(province, !!sym(var_sel)),
      type="bar", orientation="h",
      marker=list(color=val,
        colorscale=if(var_sel=="gini")
          list(list(0,"#BBF7D0"),list(1,"#DC2626"))
        else list(list(0,"#BBF7D0"),list(1,"#059669")),
        showscale=FALSE, line=list(width=0)),
      hovertemplate=paste0("<b>%{y}</b><br>",lbl,": %{x}<extra></extra>")) %>%
    layout(xaxis=list(title=lbl), yaxis=list(title=""), bargap=.25)
    plotly_cfg(p)
  })
